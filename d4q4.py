from jeuSudoku_MQ import verifierLigne,verifierCol,verifierSousGrille,verifierGagner,verifierValide
...
def afficherGrille(grille):
    ...
    while col < len(grille):
      '''print(col, end="  ")'''#Ce print ajoute une rangee
      col += 1
    print()
    row = 0
    while row < len(grille):
       '''print(row, end="")'''#Ce print ajoute une colonne
       col = 0
...
while choix < 3 and choix > 0:
      if choix == 1:
          ...
          grille = [
              ...
              [9, 8, 3, 1, 2, 5, 6, 7, 4],
              [2, 5, 0, 8, 4, 6, 9, 3, 1]
          ]  # la seule matrice utilisé dans le programme.
      ...
      if grille[row][col] == 0:
          ajout = verifierValide(grille, row, col, num)
          if ajout == True:
              grille[row][col] = num
              print("Bravo!!")
          else:
